...
> 以下问题不影响 V1 日常使用，但应在本版本解决。

### 8.1 测试覆盖

| 优先级 | 任务 | 状态 |
|--------|------|------|
| P1 | `TodayPage` smoke test（渲染不崩溃） | ⬜ |
| P1 | `DiaryEditPage` 保存逻辑测试 | ⬜ |
| P2 | `DayCounter` 模型 `daysUntil` / `labelText` 单元测试 | ⬜ |

### 8.2 错误处理

| 优先级 | 任务 | 状态 |
|--------|------|------|
| P1 | 全局 `ErrorBoundary` widget — 捕获异步错误，显示 SnackBar | ⬜ |
| P2 | `AsyncValue.error` 统一处理 — 替代 `SizedBox.shrink()` 静默吞错 | ⬜ |

### 8.3 用户体验

| 优先级 | 任务 | 状态 |
|--------|------|------|
| P2 | 路由守卫 — 编辑页退出时提示"有未保存内容"（GoRouter `onExit`） | ⬜ |
| P2 | 日记页增加长按删除 | ⬜ |
...
| 优先级 | 任务 | 状态 |
|--------|------|------|
| P1 | 数据导出 — SQLite → JSON 文件到公共目录 | ⬜ |
| P1 | 数据导入 — JSON → SQLite 恢复 | ⬜ |
